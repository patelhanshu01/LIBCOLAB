...
import { BookOpen, TrendingUp, CheckCircle, Clock, Plus, Pencil, Trash2, X } from 'lucide-react';
...
const LibrarianDashboard = () => {
  ...
  const handleApproveBorrow = async (borrowId) => {
    try {
      ...
      const res = await axios.get(`${API}/borrows`, { headers });
      setBorrows(res.data);
    } catch (error) {
  ...
  const handleReturnBook = async (borrowId) => {
    try {
      ...
      const res = await axios.get(`${API}/borrows`, { headers });
      setBorrows(res.data);
      // Refresh books for updated counts
      ...
      <div className="space-y-6" data-testid="librarian-dashboard">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage inventory and borrow requests</p>
          </div>
        ...
        <Tabs defaultValue="pending">
          <TabsList>
            <TabsTrigger value="pending" className="gap-2" data-testid="pending-tab">
              <Clock className="w-4 h-4" /> Pending ({pendingBorrows.length})
            </TabsTrigger>
            <TabsTrigger value="active" className="gap-2" data-testid="active-tab">
              <CheckCircle className="w-4 h-4" /> Active ({activeBorrows.length})
            </TabsTrigger>
            <TabsTrigger value="inventory" className="gap-2" data-testid="inventory-tab">
              <BookOpen className="w-4 h-4" /> Inventory ({books.length})
            </TabsTrigger>
          </TabsList>
                    ...
                    <TableBody>
                      {activeBorrows.map((borrow) => (
                        <TableRow key={borrow.id}>
                          ...
                          <TableCell>
                            <Button size="sm" variant="outline" onClick={() => handleReturnBook(borrow.id)} data-testid={`return-btn-${borrow.id}`}>Mark Returned</Button>
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                        ...
                        <TableRow key={book.id}>
                          ...
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-2">
                              <Button size="sm" variant="ghost" onClick={() => openEditBook(book)} data-testid={`edit-book-${book.id}`}>
                              ...
                              {deleteConfirm === book.id ? (
                                ...
                                <Button size="sm" variant="ghost" className="text-red-500 hover:text-red-700" onClick={() => setDeleteConfirm(book.id)} data-testid={`delete-book-${book.id}`}>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <Label>Title *</Label>
                  <Input value={bookForm.title} onChange={(e) => setBookForm(p => ({ ...p, title: e.target.value }))} placeholder="Book title" data-testid="book-title-input" />
                </div>
                <div className="col-span-2">
                  <Label>Author *</Label>
                  <Input value={bookForm.author} onChange={(e) => setBookForm(p => ({ ...p, author: e.target.value }))} placeholder="Author name" data-testid="book-author-input" />
                </div>
                <div className="col-span-2">
                  <Label>Description</Label>
                  <Textarea value={bookForm.description} onChange={(e) => setBookForm(p => ({ ...p, description: e.target.value }))} placeholder="Book description" rows={2} data-testid="book-description-input" />
                </div>
                <div>
                ...
                {bookForm.pricing_type !== 'free' && (
                  <div>
                    <Label>Price ($)</Label>
                    <Input type="number" step="0.01" value={bookForm.price} onChange={(e) => setBookForm(p => ({ ...p, price: e.target.value }))} data-testid="book-price-input" />
                  </div>
                )}
                <div>
                  <Label>Format</Label>
                ...
                {bookForm.format !== 'digital' && (
                  <>
                    <div>
                      ...
                      <Input type="number" min="0" value={bookForm.available_copies} onChange={(e) => setBookForm(p => ({ ...p, available_copies: e.target.value }))} data-testid="book-available-input" />
                    </div>
                    <div>
                      <Label>Total Copies</Label>
                      <Input type="number" min="1" value={bookForm.total_copies} onChange={(e) => setBookForm(p => ({ ...p, total_copies: e.target.value }))} data-testid="book-total-input" />
                    </div>
                    <div>
                      <Label>Shelf Location</Label>
                      <Input value={bookForm.shelf_location} onChange={(e) => setBookForm(p => ({ ...p, shelf_location: e.target.value }))} placeholder="e.g., A1-01" data-testid="book-shelf-input" />
                    </div>
                  </>
                )}
                <div>
                  <Label>ISBN</Label>
                  <Input value={bookForm.isbn} onChange={(e) => setBookForm(p => ({ ...p, isbn: e.target.value }))} placeholder="Optional" data-testid="book-isbn-input" />
                </div>
              </div>
              ...
              <div>
                ...
                {bookForm.subjects.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-2">
                    {bookForm.subjects.map(s => (
                      <Badge key={s} variant="secondary" className="gap-1">
                        {s} <button onClick={() => removeSubject(s)} className="hover:text-destructive"><X className="w-3 h-3" /></button>
                      </Badge>
                    ))}
                  </div>
                )}
              ...
              <div>
                ...
                {bookForm.grade_levels.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-2">
                    {bookForm.grade_levels.map(g => (
                      <Badge key={g} variant="secondary" className="gap-1">
                        Grade {g} <button onClick={() => removeGrade(g)} className="hover:text-destructive"><X className="w-3 h-3" /></button>
                      </Badge>
                    ))}
                  </div>
                )}
              ...
              <Button onClick={handleSaveBook} className="w-full" data-testid="save-book-btn">
