...
import { Search, Filter, BookOpen, Download, MapPin, ShoppingCart, Check } from 'lucide-react';
...
const BooksPage = () => {
  ...
  const filteredBooks = books.filter(book => {
    if (subjectFilter !== 'all' && !book.subjects.includes(subjectFilter)) return false;
    return true;
  });
  ...
  const BookCard = ({ book }) => {
    ...
    return (
      <Card className="book-card overflow-hidden group" data-testid={`book-card-${book.id}`}>
        <div className="aspect-[3/4] bg-gradient-to-br from-slate-100 to-slate-200 relative overflow-hidden">
          {book.cover_image ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <BookOpen className={`w-16 h-16 ${isAcademic ? 'text-teal-300' : 'text-orange-300'}`} />
            </div>
          )}
        ...
        <CardContent className="p-4">
          ...
          <div className="flex items-center gap-2 mb-4 text-xs text-muted-foreground">
            ...
            {book.format === 'physical' || book.format === 'both' ? (
              <span className="flex items-center gap-1">
                <MapPin className="w-3 h-3" /> {book.shelf_location || 'Physical'}
              </span>
            ) : null}
          </div>
          ...
          {isAcademic ? (
            ...
            <div className="space-y-2">
              ...
              <Button 
                className={inCart ? 'w-full bg-green-600 hover:bg-green-700' : 'w-full leisure-button'}
                onClick={() => !inCart && handleAddToCart(book)}
                disabled={inCart}
  ...
  const content = (
    <div className="space-y-6" data-testid="books-page">
      ...
      {/* Tabs */}
      <Tabs value={category} onValueChange={(val) => setSearchParams({ category: val })}>
        <TabsList className="grid w-full max-w-md grid-cols-2">
          <TabsTrigger value="academic" className="gap-2" data-testid="academic-tab">
            <BookOpen className="w-4 h-4" />
            Academic (Free)
          </TabsTrigger>
          <TabsTrigger value="leisure" className="gap-2" data-testid="leisure-tab">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              {filteredBooks.map((book, index) => (
                <div key={book.id} className={`animate-fadeInUp stagger-${(index % 5) + 1}`}>
                  <BookCard book={book} />
                </div>
              ))}
            </div>
